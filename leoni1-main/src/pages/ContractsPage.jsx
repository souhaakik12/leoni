...
function computeDaysRemaining(dateFin) {
  const endDate = parseDateFr(dateFin);
...
export default function ContractsPage() {
  ...
  const dynamicSignedRows = useMemo(
    () =>
      signedContracts.map((candidate) => {
        const dateFin = candidate.dateFinContrat || candidate.dateFin || "-";
        return {
          id: `cand-${candidate.id}`,
          candidateId: candidate.id,
          fromCandidate: true,
          nom: candidate.nomComplet || "-",
          matricule: candidate.matricule || `EMP${String(candidate.id).padStart(3, "0")}`,
          type: candidate.typeContrat || "CDD",
          departement:
            candidate.departementContrat || candidate.departement || candidate.posteVise || "A definir",
          dateFin,
          joursRestants: computeDaysRemaining(dateFin),
        };
      }),
    [signedContracts]
  );
    ...
    () =>
      seedContracts.map((row) => {
        const linked = candidats.find(
          (candidate) =>
            norm(candidate.nomComplet) === norm(row.nom) ||
            (candidate.matricule && row.matricule && candidate.matricule === row.matricule)
        );
        return { ...row, candidateId: linked?.id || null };
      }),
    [candidats]
  );
  ...
  const alertes = useMemo(() => contractsData.filter((c) => c.joursRestants !== null), [contractsData]);
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 3 }}>
            Gerez les contrats des employes
          </p>
        </div>
        <button
          style={{
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
          <div>
            <strong style={{ color: "#1d4ed8", fontSize: 13 }}>Contrats signes: {signedContracts.length}</strong>
            <p style={{ fontSize: 12, color: "#365a9b", marginTop: 3 }}>
              Les candidats signes apparaissent directement dans la liste des contrats.
            </p>
          </div>
          <button
            onClick={() => navigate("/contracts/reception")}
            style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <span style={{ fontWeight: 700, color: "#ea580c", fontSize: 13 }}>
                {alertes.length} contrats proches d'expiration
              </span>
              <p style={{ fontSize: 12, color: "#c2410c", marginTop: 1 }}>Verifiez les alertes de renouvellement</p>
            </div>
          </div>
          <button
            onClick={() => setTab("alertes")}
            style={{ background: "none", border: "none", color: "#ea580c", fontSize: 13, fontWeight: 600 }}
          >
            Voir les alertes -&gt;
          </button>
        </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "#f9fafb", borderBottom: "1px solid var(--border)" }}>
                {["NOM", "MATRICULE", "TYPE", "DEPARTEMENT", "DATE FIN", "ACTIONS"].map((h) => (
                  <th
                    key={h}
                    style={{
                      padding: "11px 16px",
            ...
            <tbody>
              {contractsData.map((c, i) => (
                <tr
                  key={c.id}
                  style={{ borderBottom: i < contractsData.length - 1 ? "1px solid var(--border)" : "none" }}
                  onMouseEnter={(e) => (e.currentTarget.style.background = "#f9fafb")}
                  onMouseLeave={(e) => (e.currentTarget.style.background = "#fff")}
                >
                  <td style={{ padding: "13px 16px", fontWeight: 600, color: "var(--text-primary)", fontSize: 13 }}>
                    {c.nom}
                  </td>
                  ...
                  <td
                    style={{
                      padding: "13px 16px",
                      color: c.joursRestants !== null ? "#ea580c" : "var(--text-muted)",
                      fontSize: 13,
                      fontWeight: c.joursRestants !== null ? 600 : 400,
                    }}
                  >
                    {c.dateFin}
                  </td>
                  <td style={{ padding: "13px 16px" }}>
                    <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                      <button
                        onClick={() => openCandidateDossier(c)}
                        style={{
                          border: "1px solid #1d4ed8",
                          background: "#1d4ed8",
                          color: "#fff",
                      ...
                      <button
                        title="Consulter dossier candidat"
                        onClick={() => openCandidateDossier(c)}
                        style={{
                          background: "none",
                          border: "none",
            ...
            >
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 40px", marginBottom: 14 }}>
                <div style={{ fontSize: 13, color: "var(--text-secondary)" }}>
                  Matricule: <strong style={{ color: "var(--text-primary)" }}>{c.matricule}</strong>
                </div>
                <div style={{ fontSize: 13, color: "var(--text-secondary)" }}>
                  Departement: <strong style={{ color: "var(--text-primary)" }}>{c.departement}</strong>
                </div>
                <div style={{ fontSize: 13, color: "var(--text-secondary)" }}>
                  Type: <strong style={{ color: "var(--text-primary)" }}>{c.type}</strong>
                </div>
                <div style={{ fontSize: 13, color: "var(--text-secondary)" }}>
                  Date fin: <strong style={{ color: "#ea580c" }}>{c.dateFin}</strong>
                </div>
              </div>
              <div
                style={{
