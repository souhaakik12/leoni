...
export default function ContratsDashboard() {
  ...
  const [newContrat, setNewContrat] = useState({ nom: "", matricule: "", type: "CDD", departement: "", dateFin: "" });
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 3 }}>Gestion des contrats des employés</p>
        </div>
      ...
      <div style={{ display: "flex", gap: 14, marginBottom: 20, flexWrap: "wrap" }}>
        {[
          ...
          { label: "CDI",     value: contractsData.filter(c => c.type === "CDI").length,      color: "#16a34a", bg: "#dcfce7", border: "#bbf7d0" },
          { label: "CDD",     value: contractsData.filter(c => c.type === "CDD").length,      color: "#7c3aed", bg: "#f5f3ff", border: "#ddd6fe" },
      ...
      {/* Alert banner */}
      {alertes.length > 0 && (
        <div style={{ background: "#fff7ed", border: "1px solid #fed7aa", borderRadius: 10, padding: "12px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 12, color: "#c2410c" }}>Vérifiez les alertes de renouvellement</p>
            </div>
          </div>
      ...
      <div style={{ display: "flex", borderBottom: "2px solid var(--border)", marginBottom: 24 }}>
        {[
          ...
          <button key={t.key} onClick={() => setTab(t.key)} style={{
            ...
            {t.label}
            {t.count && (
              <span style={{ background: "#dc2626", color: "#fff", borderRadius: "50%", width: 18, height: 18, fontSize: 10, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>
                {t.count}
              </span>
            )}
          </button>
            ...
            <tbody>
              {contractsData.map((c, i) => (
                <tr key={c.id} style={{ borderBottom: i < contractsData.length - 1 ? "1px solid var(--border)" : "none" }}
                  ...
                  <td style={{ padding: "13px 16px", fontWeight: 600, fontSize: 13 }}>{c.nom}</td>
                  <td style={{ padding: "13px 16px", fontSize: 13, color: "var(--text-secondary)" }}>{c.matricule}</td>
                  <td style={{ padding: "13px 16px" }}>
                    <span style={{ background: c.type === "CDI" ? "#eff6ff" : "#fff7ed", color: c.type === "CDI" ? "#2563eb" : "#ea580c", borderRadius: 6, padding: "2px 9px", fontSize: 12, fontWeight: 600 }}>{c.type}</span>
                  </td>
                  <td style={{ padding: "13px 16px", fontSize: 13, color: "var(--text-secondary)" }}>{c.departement}</td>
                  <td style={{ padding: "13px 16px", fontSize: 13, fontWeight: c.joursRestants ? 600 : 400, color: c.joursRestants ? "#ea580c" : "var(--text-muted)" }}>{c.dateFin}</td>
                  <td style={{ padding: "13px 16px" }}>
                    <div style={{ display: "flex", gap: 8 }}>
                      ...
                      {c.joursRestants && (
                        <button style={{ background: "#fff7ed", border: "1px solid #fed7aa", color: "#ea580c", borderRadius: 6, padding: "2px 8px", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
                          Renouveler
                        </button>
                      )}
                    </div>
      ...
      {tab === "alertes" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {alertes.map(c => (
            <div key={c.id} style={{ background: "#fff", border: "1px solid var(--border)", borderLeft: "4px solid #ea580c", borderRadius: 10, padding: "20px 24px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <span style={{ background: "#fff7ed", color: "#ea580c", border: "1px solid #fed7aa40", borderRadius: 99, padding: "2px 10px", fontSize: 12, fontWeight: 600 }}>
                    {c.joursRestants} jours restants
                  </span>
                </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 40px", marginBottom: 12 }}>
                ...
                <div style={{ fontSize: 13, color: "var(--text-secondary)" }}>Type: <strong>{c.type}</strong></div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: "#fff", borderRadius: 14, padding: "28px 32px", width: 480, boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
              ...
              <button onClick={() => setShowModal(false)} style={{ background: "none", border: "none", color: "#6b7280", fontSize: 18, cursor: "pointer" }}>✕</button>
            </div>
            {[
              { label: "Nom complet",  key: "nom",         placeholder: "Ex: Ahmed Ben Ali"    },
              { label: "Matricule",    key: "matricule",   placeholder: "Ex: EMP099"            },
              { label: "Département",  key: "departement", placeholder: "Ex: Production"        },
              ...
              <div key={f.key} style={{ marginBottom: 14 }}>
                ...
                <input value={newContrat[f.key]} onChange={e => setNewContrat(p => ({ ...p, [f.key]: e.target.value }))}
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <select value={newContrat.type} onChange={e => setNewContrat(p => ({ ...p, type: e.target.value }))}
                style={{ width: "100%", padding: "8px 12px", border: "1px solid var(--border)", borderRadius: 7, fontSize: 13, outline: "none" }}>
                <option>CDD</option><option>CDI</option>
              </select>
            </div>
            <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
              <button onClick={() => setShowModal(false)} style={{ flex: 1, padding: "9px", border: "1px solid var(--border)", borderRadius: 8, background: "#f9fafb", color: "var(--text-secondary)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Annuler</button>
              <button onClick={() => setShowModal(false)} style={{ flex: 1, padding: "9px", border: "none", borderRadius: 8, background: "#2563eb", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Enregistrer</button>
            </div>
          </div>
        </div>
      )}
